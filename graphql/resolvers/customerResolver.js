...
const customerResolver = {
  ...
  Query: {
    ...
    findCustomer: safeResolver(
      async (_, { fullName, dob, email, contact_no }) => {
    ...
    search: safeResolver(async (_, { search }, { user }) => {
      ...
      const accounts = await Customer.aggregate([
        ...
        },
        {
          $project: {
            ...
            last_payment_amount: "$ca.out_standing_details.last_payment_amount",
    ...
    getMonthlyPerformance: safeResolver(async (_, __, { user }) => {
    ...
    findCustomerAccount: safeResolver(async (_, { query }, { user }) => {
      if (!user) throw new CustomError("Unauthorized", 401);
    ...
    getMonthlyTarget: safeResolver(async (_, __, { user }) => {
    ...
    customerOtherAccounts: safeResolver(async (_, { caId }) => {
      if (!caId) return null;
    ...
    masterFileAccounts: safeResolver(
      async (_, { bucketIds, page = 1, limit = 200, search }, { user }) => {
        if (!user) throw new CustomError("Unauthorized", 401);
    ...
    masterFileAccountsByCallfile: safeResolver(
      async (_, { callfileIds, page = 1, limit = 200, search }, { user }) => {
        if (!user) throw new CustomError("Unauthorized", 401);
  ...
  CustomerAccount: {
    assigned: safeResolver(async (parent) => {
