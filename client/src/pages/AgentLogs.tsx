  ...
  return (
    <div className="p-5 w-full h-[90.7%]">
      <div className="w-full flex flex-col gap-2 h-full">
        <motion.div
          className="flex flex-col border-2 border-blue-800 rounded-md bg-blue-200 h-full"
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ type: "spring" }}
        >
          <div className="w-full bg-blue-500 font-black uppercase text-white py-2 text-center border-b-2 border-blue-800">
            Agent logs
          </div>
          <div className="p-4 h-full flex flex-col gap-2">
            <div className="flex justify-between ">
              <div className="relative">
                <motion.div
                  className="bg-blue-100 justify-between cursor-pointer hover:bg-blue-200 transition-all px-3 flex gap-3 py-1 rounded-sm shadow-md border-2 border-blue-800 min-w-64"
                  onClick={() => {
                ...
                <AnimatePresence>
                  {isBucketMenuOpen && bucketOptions.length > 0 && (
                    <motion.div
                      ...
                      exit={{ y: -5, opacity: 0 }}
                      className="absolute flex flex-col max-h-80 overflow-auto z-20 border-2 border-blue-800 bg-blue-100 shadow-md transition-all cursor-pointer rounded-sm mt-1 w-full"
                    >
                      {bucketOptions.map((bucket) => (
                        <span
                          ...
                          className="whitespace-nowrap hover:bg-blue-200 px-3 py-1"
                          key={bucket._id}
                        >
                          {bucket.name}
                        </span>
              ...
              <div className="flex gap-2 font-black uppercase">
                <div className="flex items-center gap-2">
                  <div>From:</div>
                  <div className="rounded-sm bg-blue-100 border-blue-800 shadow-md border-2">
                    <input
            ...
            <div>
              <div className="grid bg-blue-500 text-white grid-cols-6 gap-2 font-black uppercase rounded-t-md border-2 border-blue-800 px-4 py-2">
                <div>vici id</div>
                <div>name</div>
                <div>total calls</div>
                <div>Average</div>
              ...
              <div className=" flex flex-col overflow-auto">
                {getAllProdData &&
                  getAllProdData.getAllAgentProductions.length === 0 && (
                    <div className="text-center border-x-2 border-b-2 border-blue-800 rounded-b-md shadow-md bg-blue-100 text-gray-400 italic py-3">
                ...
                {getAllProdData?.getAllAgentProductions.map((prod, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      key={prod.user._id}
                      className="pl-4 py-2 gap-2 even:bg-blue-100 odd:bg-blue-200 last:rounded-b-md items-center border-x-2 borderb-blue-800 grid grid-cols-6 hover:bg-blue-300 border-b-2"
                    >
                      <div>
                        {userInfo?.vici_id || (
                  ...
                  {prods?.prod_history?.map((prod, index) => (
                    ...
                    >
                      <div className="text-center border-x bg-gray-300 py-2 text-black font-bold border-t">
                        ...
                        In{" "}
                        {new Date(prod.createdAt).toLocaleTimeString("en-US", {
                          hour: "2-digit",
                          minute: "2-digit",
                          hour12: true,
                        })}
