    ...
    <div className="w-10/12 h-full overflow-y-auto overflow-x-hidden p-2 flex flex-col gap-2">
      {
        monthlyPerformance?.getMonthlyPerformance.map((e)=> {
          ...
          return (  
            <div key={e.campaign}  className="border flex h-50 w-full rounded-xl border-slate-200 shadow shadow-black/20 bg-white hover:scale-105 duration-500 ease-in-out hover:left-5 hover:ml-10 cursor-pointer" onClick={()=> {setCampaign(deptId[e.campaign]); setReportVariables(e.campaign)}}>
                  ...
                  <div className="h-full w-full pt-2 grid grid-cols-2 grid-rows-2">
                    
                    {
                      ...
                      <div key={index} className="flex flex-col gap-1 ">
                        ...
                        <div className={`w-4/6 border border-${ps.color}-400 rounded-full h-5 shadow-xl/30 shadow-${ps.color}-500 relative`}>
                          <div style={{width: `${ ps.key1 !== "attendanceRate" ? ((value1 / value2 ) * 100) < 9 ? 9 : ((value1 / value2 ) * 100) : value1 }%`}} className={`h-full bg-${ps.color}-400 w-3/4 rounded-full border border-${ps.color}-300 flex justify-end cursor-default peer duration-700 ease-in`}>
                            <p className="text-[0.5em]  font-medium text-slate-800 px-2 rounded-full bg-white  flex items-center justify-center ">{ ps.key1 !== "attendanceRate" ? ((value1 /value2) * 100).toFixed(2): value1.toFixed(2)}%</p>
                          </div>
                          {
                            ps.key1 !== "attendanceRate" &&
                            <div className={`peer-hover:flex hidden absolute bg-white rounded-xl text-xs py-1 px-3 -translate-y-1/2 -top-2/2 -translate-x-1/2 left-1/2 border border-slate text-red-400 font-bold `}>{value1} / {value2}</div>
                          }
                        </div>
                      </div>
                      )
