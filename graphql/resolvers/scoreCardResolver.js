...
const scoreCardResolver = {
  JSON: GraphQLJSON,
  Query: {
    getScoreCardSummaries: safeResolver(async (_, { date, search }) => {
      const query = {};
      if (date) {
  ...
  Mutation: {
    createScoreCardData: safeResolver(async (_, { input }, { user }) => {
      if (!user) {
    ...
    createUBScoreCardData: safeResolver(async (_, { input }, context) => {
