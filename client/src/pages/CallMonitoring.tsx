...
type BucketSource = "tl" | "all";

type CallMonitoringVariant = "tl" | "qa" | "admin" | "qasv";

type CallMonitoringProps = {
  variant: CallMonitoringVariant;
};
...
const ALL_BUCKET_OPTION: BucketBase = {
  _id: "ALL",
  dept: "All",
...
const variantConfig: Record<
  CallMonitoringVariant,
  {
    ...
    showViciIp?: boolean;
    bargeGuard?: boolean;
    bargeRefetchOnComplete?: boolean;
    allowBargeWhenSession?: boolean;
...
> = {
  ...
  admin: {
    ...
    paddingXClass: "px-10",
    showViciIp: true,
    allowBargeWhenSession: true,
    bargeRefetchOnComplete: true,
  },
  qasv: {
    bucketSource: "all",
    allowAllOption: true,
...
const CallMonitoring = ({ variant }: CallMonitoringProps) => {
  const {
    bucketSource,
    ...
    showViciIp = false,
    bargeGuard = false,
    bargeRefetchOnComplete = false,
    allowBargeWhenSession = false,
    emptyAllMessage = "Select a bucket to view agents",
  } = variantConfig[variant];

  const [selectedBucket, setSelectedBucket] = useState<BucketBase | null>(
    allowAllOption ? ALL_BUCKET_OPTION : null,
  );
  const [hoveredRow, setHoveredRow] = useState<number | null>(null);
  const [isOpen, setIsOpen] = useState(false);

  const tlBuckets = useQuery<{ getTLBucket: BucketBase[] }>(GET_TL_BUCKET, {
    skip: bucketSource !== "tl",
    notifyOnNetworkStatusChange: true,
  });

  const allBuckets = useQuery<{ getAllBucket: BucketBase[] }>(GET_ALL_BUCKET, {
    skip: bucketSource !== "all",
    notifyOnNetworkStatusChange: true,
  });
  ...
  const buckets = useMemo(() => {
    const data =
      bucketSource === "tl"
        ? (tlBuckets.data?.getTLBucket ?? [])
        : (allBuckets.data?.getAllBucket ?? []);

    if (!allowAllOption) return data;
    return [ALL_BUCKET_OPTION, ...data];
  }, [bucketSource, tlBuckets.data, allBuckets.data, allowAllOption]);

  useEffect(() => {
    if (selectedBucket) return;
    if (buckets.length === 0) return;
    setSelectedBucket(allowAllOption ? ALL_BUCKET_OPTION : buckets[0]);
  }, [buckets, selectedBucket, allowAllOption]);

  const isAll = Boolean(selectedBucket?.isAll);

  const { data: bucketUsersData } = useQuery(GET_BUCKET_USERS, {
    variables: selectedBucket ? { bucketId: selectedBucket._id } : undefined,
    skip: !selectedBucket || isAll,
    notifyOnNetworkStatusChange: true,
  });
  ...
  const allowedViciIds = useMemo(() => {
    return new Set(
      bucketUsers
        .map((user: any) => user.vici_id?.trim())
        .filter((id: string | undefined): id is string => Boolean(id)),
    );
  }, [bucketUsers]);

  const { data: callLogsData } = useQuery<{ getUsersLogginOnVici: string }>(
    CALL_LOGS,
    {
      notifyOnNetworkStatusChange: true,
      variables: selectedBucket ? { bucket: selectedBucket._id } : undefined,
      skip: !selectedBucket || isAll,
      pollInterval: isAll ? undefined : 1000,
    },
  );
  ...
  const { data: getUserData, refetch: getUserDataRefetch } = useQuery<{
    ...
    variables: { viciId: selectedBucket?.viciIp || null },
    skip: !selectedBucket?.viciIp || (!bargeGuard && !bargeRefetchOnComplete),
    notifyOnNetworkStatusChange: true,
  });
  ...
  }, [rows, bucketUsersData, allowedViciIds, isAll]);

  const [bargeCall] = useMutation(BARGE_CALL, {
    onCompleted: async () => {
      if (bargeRefetchOnComplete && getUserDataRefetch) {
  ...
  const handleBargeCall = useCallback(
    async (session_id: string | null, viciUserId: string | null) => {
      if (bargeGuard) {
        if (getUserData?.getBargingStatus?.includes("ERROR")) return;
      }
      await bargeCall({
        variables: {
    ...
    [bargeCall, getUserData, bargeGuard],
  );

  return (
    <div
      className={`w-full relative ${paddingXClass} gap-2 pt-2 pb-5 h-[91vh] flex flex-col`}
    >
      <motion.div
        className="bg-blue-200 gap-2 border-2 border-blue-800 rounded-md shadow-md flex flex-col h-full"
        initial={{ y: 20, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
      >
        <div className="flex w-full py-2 bg-blue-500 font-black uppercase text-white text-center justify-center border-b-2 border-blue-800">
          Call monitoring
        </div>
        <div className="p-4 flex flex-col gap-2" >
            ...
            <div className="flex items-center gap-3">
              <div>
                <motion.div onClick={() => setIsOpen(!isOpen)} layout>
                  <div className="bg-blue-100 relative z-20 cursor-pointer hover:bg-blue-300 transition-all px-2 flex gap-3 py-1 rounded-sm shadow-md border-2 border-blue-800">
                ...
                <AnimatePresence>
                  {isOpen && (
                    <motion.div
                      ...
                      exit={{ y: -5, opacity: 0 }}
                    >
                      <div className="absolute flex flex-col max-h-80 overflow-auto z-20 border-2 border-blue-800 shadow-md bg-blue-100 transition-all cursor-pointer rounded-sm mt-1">
                        {buckets.map((bucket) => (
                          <span
                            onClick={() => {
                            ...
                            className="whitespace-nowrap border-b border-blue-300 hover:bg-blue-300  even:bg-blue-200 odd:bg-blue-100 px-3 py-1"
                            key={bucket._id}
                          >
                            {bucket.name}
                          </span>
              ...
              {showViciIp && (
                <motion.div
                  ...
                  transition={{ delay: 1 }}
                >
                  {isAll ? "Select a Bucket" : selectedBucket?.viciIp}
                </motion.div>
              )}
            </div>

            <div className="bg-gray-200 rounded-sm shadow-md border-2 border-blue-800 items-center gap-3 px-3 py-1 flex">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
          ...
          <motion.div className="flex flex-col h-full overflow-auto">
            <div className="grid font-black text-sm uppercase bg-blue-500 text-white border-blue-800 border-2 px-4 gap-2 py-2 rounded-t-md grid-cols-13">
              <div>User</div>
            ...
            <div className="w-full flex flex-col h-full overflow-auto">
              {isAll ? (
                <div className="flex justify-center items-center bg-blue-100 w-full py-3 rounded-b-md shadow-md italic text-gray-400 border-blue-800 border-x-2 border-b-2">
                  <div>{emptyAllMessage}</div>
                </div>
                ...
                filteredRows.map((x, index) => {
                  ...
                  const canBargeStrict =
                    x.includes("INCALL") &&
                    !x.includes("DEAD") &&
                    !x.includes("DISPO") &&
                    !x.includes("DIAL");
                  const canBarge = allowBargeWhenSession
                    ? canBargeStrict || Boolean(session)
                    : canBargeStrict;

                  return (
                    <motion.div
                      key={index}
                      className="grid pl-4 pr-1 gap-2 hover:bg-blue-600 last:shadow-md last:rounded-b-md even:bg-blue-50 odd:bg-blue-100 border-blue-800 border-x-2 border-b-2 grid-cols-13"
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                                ...
                                >
                                  ...
                                  >
                                    <path
                                      strokeLinecap="round"
                                      strokeLinejoin="round"
                                      d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 0 0 2.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 0 1-7.143-7.143c-.162-.441.004-.928.380-1.210l1.293-.970c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 0 0-1.091-.852H4.5A2.25 2.25 0 0 0 2.25 4.5v2.25Z"
                                    />
                                  </svg>
                                </motion.div>
                              </AnimatePresence>
                      ...
                      <div>
                        {!canBarge ? (
                          ...
                          <span
                            ...
                            onClick={() => handleBargeCall(session, viciId)}
                            title="barge"
                          >
                            <div
