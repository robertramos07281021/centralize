...
const CallAllAgentLogs = () => {
  ...
  const allBuckets = allBucketsData?.getAllBucket ?? [];

  useEffect(() => {
    if (!selectedBucket && allBuckets.length) setSelectedBucket(allBuckets[0]);
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      return viciId ? allowedViciIds.has(viciId) : false;
    });
          ...
          {filteredRows.length === 0 ? (
            ...
            filteredRows.map((x, index) => {
              ...
              return (
                <motion.div
                  key={index}
                  className="grid px-4 gap-2 hover:bg-gray-3 00 last:shadow-md last:rounded-b-md even:bg-gray-100 odd:bg-gray-200 border-x border-b grid-cols-13"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
