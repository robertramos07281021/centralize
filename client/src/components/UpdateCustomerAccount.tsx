...
const UpdateCustomerAccount:React.FC<CustomerProps> = ({cancel}) => {
  ...
  const [updateCustomerAccount] = useMutation<{updateCustomerAccount:UpdatedCusterAccount}>(UPDATE_ACCOUNT_INFO, {
    onCompleted: (data)=> {
      ...
      if(selectedCustomer) {
        dispatch(setSelectedCustomer({...selectedCustomer, balance: data.updateCustomerAccount.customerAccount.balance,out_standing_details: data.updateCustomerAccount.customerAccount.out_standing_details, account_update_history: data.updateCustomerAccount.customerAccount.account_update_history  }))
      }
      cancel()
    },
    onError: ()=> {
  ...
  const handleOnChangeAmount = useCallback((e:React.ChangeEvent<HTMLInputElement>,toUpdate: keyof typeof formData) => {
  ...
  const onSubmit = useCallback(()=> {
    ...
    setModalProps({
      ...
      toggle: "UPDATE",
      yes: async() => {
        await updateCustomerAccount({variables: {input: {id: selectedCustomer?._id, total_os: Number(formData.total_os), principal_os: Number(formData.principal), balance: Number(formData.balance)}}})
        setConfirm(prev => !prev)
      },
      no: () => {
        setConfirm(prev => !prev)
      ...
      <fieldset className="flex flex-col gap-5 border w-full border-slate-500 rounded-xl p-2 bg-white">
        ...
        <div className="flex lg:gap-4 2xl:gap-10 items-center 2xl:text-base text-xs lg:text-sm flex-col 2xl:flex-row justify-center">
          ...
          <label>
            ...
            <input type="text" className="border rounded-lg p-2 border-slate-400" value={formData.principal || ""} onChange={(e) => handleOnChangeAmount(e,"principal")}/>
          </label>
          <label>
            <p className="font-medium text-gray-600">Balance :</p>
            <input type="text" value={formData.balance || ""} className="border rounded-lg p-2 border-slate-400" onChange={(e) => handleOnChangeAmount(e,"balance")}/>
          </label>
        </div>
        <div className="flex gap-10 items-center justify-center pb-5">
          <button className="py-2.5 hover:bg-orange-600 bg-orange-400 font-medium rounded-lg text-sm cursor-pointer w-22 text-white" onClick={onSubmit}>Save</button>
          <button className="py-2.5 hover:bg-slate-600 bg-slate-400 font-medium rounded-lg text-sm cursor-pointer w-22 text-white" onClick={()=> cancel()}>Cancel</button>
        </div>
      </fieldset>
      { confirm &&
