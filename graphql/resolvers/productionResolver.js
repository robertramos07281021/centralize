...
const productionResolver = {
  DateTime,
  Query: {
    productions: safeResolver(async () => {
      const data = await Production.find().lean();
      return data;
    }),
    productionByUser: safeResolver(async (_, { userId }) => {
      const prod = await Production.findOne({ user: userId }).lean();
      return prod;
    }),
    getAgentProductionPerDay: safeResolver(async (_, __, { user }) => {
    ...
    agentProduction: safeResolver(async (_, __, { user }) => {
      if (!user) throw new CustomError("Unauthorized", 401);
      const now = new Date();
    ...
    getAgentProductionPerMonth: safeResolver(async (_, __, { user }) => {
      if (!user) throw new CustomError("Unauthorized", 401);
      const year = new Date().getFullYear();
    ...
    getAgentTotalDispositions: safeResolver(
      async (_, { from, to }, { user }) => {
        if (!user) throw new CustomError("Unauthorized", 401);
        const now = new Date();
    ...
    getAgentDailyCollection: safeResolver(async (_, __, { user }) => {
      if (!user) throw new CustomError("Unauthorized", 401);
    ...
    ProductionReport: safeResolver(
      async (_, { dispositions, from, to }, { user }) => {
        if (!user) throw new CustomError("Unauthorized", 401);
    ...
    getAgentProductions: safeResolver(async (_, __, { user }) => {
      if (!user) throw new CustomError("Unauthorized", 401);
        ...
        },
        {
          $match: {
            "userInfo.departments": {
              $in: user.departments.map((e) => new mongoose.Types.ObjectId(e)),
            },
    ...
    getAllAgentProductions: safeResolver(async (_, { bucketId, from, to }) => {
      ...
      const users = (await User.find({ buckets: bucketId, type: "AGENT" })).map(
    ...
    ptpToConfirmPaid: safeResolver(async (_, { bucket, interval }) => {
    ...
    confirmPaid: safeResolver(async (_, { bucket, interval }) => {
      const selectedBucket = await Bucket.findById(bucket).lean();
    ...
    getAgentDispositionRecords: safeResolver(
      async (
        _,
    ...
    checkAgentIfHaveProd: safeResolver(async (_, { bucket, interval }) => {
      const selectedBucket = await Bucket.findById(bucket).lean();
    ...
    monthlyWeeklyCollected: safeResolver(async (_, __, { user }) => {
      const now = new Date();
      const currentDay = now.getDay();
      const diffToMonday = currentDay === 0 ? -6 : 1 - currentDay;
    ...
    getAgentRPCCount: safeResolver(async (_, __, { user }) => {
  ...
  DipotypeCount: {
    dispotype: safeResolver(async (parent) => {
      const dispotypes = await DispoType.findById(parent.dispotype);
      return dispotypes;
    }),
  },
  Mutation: {
    setTargets: safeResolver(async (_, { userId, targets }) => {
    ...
    loginToProd: safeResolver(async (_, { password }, { user }) => {
      if (!user) throw new CustomError("Unauthorized", 401);
    ...
    setBucketTargets: safeResolver(async (_, { bucketId, targets }) => {
      const { daily, weekly, monthly } = targets;
    ...
    lockAgent: safeResolver(async (_, __, { user, pubsub, PUBSUB_EVENTS }) => {
