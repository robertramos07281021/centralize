...
const taskResolver = {
  Query: {
    myTasks: safeResolver(async (_, __, { user }) => {
    ...
    groupTask: safeResolver(async (_, __, { user }) => {
      if (!user) throw new CustomError("Unauthorized", 401);
  ...
  Mutation: {
    selectTask: safeResolver(
      async (_, { id }, { user, pubsub, PUBSUB_EVENTS }) => {
        if (!user) throw new CustomError("Unauthorized", 401);
        ...
        if (ca.on_hands) {
          ...
          if (
            customerAgent &&
            ca?._id?.toString() === customerAgent?.handsOn?.toString()
          ) {
    ...
    deselectTask: safeResolver(async (_, { id }, { PUBSUB_EVENTS, pubsub }) => {
    ...
    tlEscalation: safeResolver(
      async (_, { id, tlUserId }, { PUBSUB_EVENTS, pubsub }) => {
