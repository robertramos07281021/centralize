...
const callResolver = {
  Query: {
      ...
      const pipeline = [
        ...
        // --- Lookup callfile to get roundCount if autoDial ---
        {
          $lookup: {
            ...
            foreignField: "_id",
            as: "ac",
            pipeline: [{ $project: { roundCount: 1, autoDial: 1, active: 1 } }],
          },
        },
        { $unwind: "$ac" },
    ...
    checkIfAgentIsInline: safeResolver(async (_, __, { user }) => {
    ...
    getUsersLogginOnVici: safeResolver(async (_, { bucket }) => {
    ...
    isAutoDial: safeResolver(async (_, __, { user }) => {
      if (!user) throw new CustomError("Unauthorized", 401);
    ...
    checkIfCallfileAutoIsDone: safeResolver(async (_, { callfile }) => {
    ...
    getBargingStatus: safeResolver(async (_, { vici_id }, { user }) => {
      if (!user) throw new CustomError("Unauthorized", 401);
  ...
  Mutation: {
    makeCall: safeResolver(async (_, { phoneNumber }, { user }) => {
      if (!user) throw new CustomError("Unauthorized", 401);
    ...
    lateCallRecording: safeResolver(async (_, { id }, { user }) => {
      if (!user) throw new CustomError("Unauthorized", 401);
