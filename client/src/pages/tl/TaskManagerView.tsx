      ...
      <div className="flex gap-10 p-5 items-start">
        <div className=" flex gap-5 lg:text-[0.6em] 2xl:text-xs w-full flex-col">
          <div className="flex gap-5">
            <motion.fieldset
              className="flex p-1.5 gap-4 px-4 shadow-sm border rounded-md border-slate-300"
              initial={{ y: 20, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              transition={{ delay: 0.8, duration: 0.5, type: "spring" }}
            >
              <legend className="font-black text-sm uppercase text-slate-600 px-2">
                Tasker
              </legend>
            ...
            >
              <legend className="font-black uppercase  text-sm text-slate-600 px-2">
                Tasks Filter
              </legend>
              <label className="text-sm font-medium text-gray-900 flex items-center gap-1">
                <input
          ...
          <div className="flex gap-5">
            <motion.div
              className="w-1/2 border cursor-pointer shadow-sm  rounded-md h-10 border-slate-300 relative z-50"
            ...
            >
              ...
              <div
                className="lg:w-60  2xl:w-80 h-full px-2 font-bold uppercase truncate text-slate-500 flex text-sm items-center"
                onClick={onClick}
              >
              ...
              {showSelection && (
                <div className="w-full h-96  border overflow-y-auto absolute top-10 flex rounded-md mt-1 shadow-md text-xs flex-col border-slate-300 bg-white">
                  {DispositionTypes?.getDispositionTypes
                    .filter((e) => e.name !== "PAID")
                    .map((e) => (
                      <label
                        key={e.id}
                        className="flex gap-2 text-black even:bg-gray-100 px-2 py-3"
                      >
                        <input
                          type="checkbox"
                          name={e.name}
            ...
            >
              <select
                className="w-full cursor-pointer focus:outline-none h-full shadow-sm text-sm border rounded-md border-slate-300 font-bold text-slate-500 px-1"
                name="bucket"
                id="bucket"
                value={bucketSelect}
              ...
              >
                {bucketData?.getTLBucket.map((e) => (
                  <option className="rounded-md mt-10" key={e._id} value={e.name}>
                    {e.name}
                  </option>
                ))}
              </select>
            ...
            >
              <label>
                <input
                  type="number"
                  className="w-15 text-sm pl-4 h-10 shadow-sm border border-slate-300 rounded-md font-bold text-slate-500 px-1"
                  name="dpd"
                  id="dpd"
                  min={0}
                  value={dpd ?? ""}
