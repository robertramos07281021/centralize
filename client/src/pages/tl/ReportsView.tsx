...
const ReportsView:React.FC<Props> = ({search}) => {
  ...
  const smsMethod = reportsData?.getDispositionReports?.toolsDispoCount?.find(x=> x.call_method === 'sms')?.dispositions || []
    ...
    <div className={` col-span-2 flex flex-col overflow-auto relative h-5/6 px-5`}>
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-5 py-5">
          {
            callMethod.length > 0 &&
            <CallReportTables totalAccounts={totalAccounts} reportsData={callMethod} callfile={callfile} />
          }
          {
            smsMethod.length > 0 &&
            <ReportsTables totalAccounts={totalAccounts} dispo={smsMethod} firstTitle="SMS Status" secondTitle="SMS Response" color="yellow" />
          }
          {
            emailMethod.length > 0 &&
            <ReportsTables totalAccounts={totalAccounts} dispo={emailMethod} firstTitle="Email Status" secondTitle="Email Response" color="indigo"/>
          }
          {
            skipMethod.length > 0 &&
