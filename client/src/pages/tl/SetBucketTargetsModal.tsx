...
const SetBucketTargetsModal: React.FC<Modal> = ({ cancel, refetch }) => {
  ...
  const { data, refetch: tlBucketRefetch } = useQuery<{
    getTLBucket: Bucket[];
  }>(BUCKETS);
  const [bucket, setBucket] = useState<string>("");
  const length = data?.getTLBucket.length && data?.getTLBucket.length > 1;
  const [setBucketTargets] = useMutation<{ setBucketTargets: Success }>(
    SET_BUCKET_TARGETS,
    {
  ...
  const handleSuccess = useCallback(async () => {
    if (bucket) {
      await setBucketTargets({
        variables: {
          bucketId: length ? bucket : data?.getTLBucket[0]._id,
          targets,
        },
      });
      setRequired(false);
          ...
          {length && (
            <label className="flex flex-col w-2/3">
              ...
              <select
                ...
                id="bucket"
                onChange={(e) => {
                  const selected = data?.getTLBucket.find(
              ...
              >
                ...
                {data?.getTLBucket.map((e) => (
                  <option key={e._id} id={e.name} value={e.name}>
                    {e.name}
                  </option>
                ))}
              </select>
