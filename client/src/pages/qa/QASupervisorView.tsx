...
const QASupervisorView = () => {
  ...
  const { data: deptBucketData, refetch: deptBucketRefetch, loading } = useQuery<{
    getDepartmentBucket: Bucket[];
  }>(GET_DEPARTMENT_BUCKET, {
    variables: { depts: userId.departments, skip:  userId?.departments?.length < 1},
  ...
  const { data: usersData, refetch:usersRefetch, loading:usersLoading} = useQuery<{
    getQAUsers: GetUser;
  }>(GET_USERS, {
  ...
  const [updateQAUser, {loading:updateLoading}] = useMutation<{updateQAUser:{success:boolean, message: string}}>(UPDATE_QA_USER, {
  ...
  const handleUpdateUser = useCallback(()=> {
    if(!userId.userId || userId.departments.length <= 0 || userId.buckets.length <= 0) {
      setRequired(true)
    } else {
      setConfirm(true)
      setRequired(false)
      setModalProps({
        ...
        toggle: "UPDATE",
        yes: async() => {
          await updateQAUser({variables: { input: { userId: userId?.userId?._id, departments: userId.departments, buckets:userId.buckets }}});
          setConfirm(false);
        },
        no: () => setConfirm(false)
      })
  ...
  return (
    ...
    <div className="relative w-full h-full">
      ...
      <div className="p-5">
        <div className="grid grid-cols-9 px-2 py-2 rounded-t-md font-black uppercase bg-gray-300">
        ...
        <div className="flex flex-col">
          {usersData?.getQAUsers?.users?.map((user, index) => (
            <motion.div
              key={user._id}
              className="grid grid-cols-9 items-center bg-gray-100 py-2 px-2 even:bg-gray-200"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              ...
              <div className="col-span-2">
                {user.departments.map((dept) => newDeptMap[dept]).join(", ")}
              </div>
              <div className="col-span-4">
                {user.buckets.map((bucket) => newBucketMap[bucket]).join(", ")}
              </div>
            ...
            >
              ...
              </div>
              {
                ...
                <h1 className="text-center text-sm text-red-500">
                  {
                    (userId.departments.length <= 0 && userId.buckets.length > 0) && 'Please select Department'
                  }
                  {
                    (userId.departments.length > 0 && userId.buckets.length <= 0) && 'Please select Buckets'
                  }
                  {
                    (userId.departments.length <= 0 && userId.buckets.length <= 0) && 'Please select Department and Bucket'
                  }
                </h1>
              }
                ...
                <div className="grid grid-cols-2 gap-3 h-full overflow-hidden">
                  <div className="overflow-auto flex flex-col gap-0">
                    {deptData?.getDepts?.map((dept) => { 
                      ...
                      <label key={dept.id} className="flex gap-2 items-center">
                        <input
                          type="checkbox"
                          id={dept.id}
                      ...
                      : (
                        userId.departments.length === 0 ? (
                          ...
                          <div className="flex  flex-col h-full overflow-auto gap-x-3 py-2 ">
                            {deptBucketData?.getDepartmentBucket?.map((bucket) => (
                              <label
                                key={bucket._id}
                              ...
                              >
                                ...
                                />
                                <label htmlFor={bucket._id}>{bucket.name}</label>
                              </label>
                            ))}
                          </div>
                        )
              ...
              <div className=" flex justify-end mt-3">
                <div className="bg-orange-500 font-medium uppercase text-white cursor-pointer hover:bg-orange-600 transition-all px-4 py-2 border-2 rounded-lg" onClick={handleUpdateUser}>
                  Update
                </div>
              </div>
            </motion.div>
