  ...
  return (
    ...
    >
      <div className="grid grid-cols-6 gap-2 col-span-8 row-span-2">
        <DailyFTE />
        <motion.div
          className="grid grid-cols-5 col-span-4 gap-2 "
      ...
      {showSelector && (
        <div
          className="absolute bottom-10 right-20 w-2/20 border bg-white rounded-md border-slate-500 p-2 flex flex-col gap-2"
          ...
          {buckets?.map((x) => {
            ...
            return (
              findBucket?.isActive && (
                <label
                  key={x}
                  className={`w-full border border-slate-400 text-gray-700 hover:bg-slate-200 cursor-pointer px-2 py-1 rounded ${
                    selectedBucket === x ? "bg-blue-200" : ""
                  }`}
                >
                  <input
                    type="radio"
        ...
        >
          {Object.entries(IntervalsTypes).map(([key, value]) => (
            <label
              key={key}
              className={`w-full border border-slate-400 text-gray-700 hover:bg-slate-200 cursor-pointer px-2 py-1 rounded ${
                value === intervalTypes ? "bg-blue-200" : ""
              }`}
            >
              <input
                type="radio"
